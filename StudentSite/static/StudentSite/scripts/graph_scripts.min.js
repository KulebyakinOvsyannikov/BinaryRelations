...
function addRelationForTableElement(a){a=a.name.split("-");addRelationIndices(parseInt(a[0]),parseInt(a[1]))}function removeRelationForTableElement(a){a=a.name.split("-");removeRelationIndices(parseInt(a[0]),parseInt(a[1]))}function removeRelation(a,b){try{graph.graph.dropEdge(String(a)+"->"+String(b)),console.log(graph),0==noRefresh&&graph.refresh()}catch(c){console.log("wrong delete")}}
function addRelation(a,b){try{graph.graph.addEdge({id:String(a)+"->"+String(b),source:String(a),target:String(b),type:a==b?"curve":"arrow"}),0==noRefresh&&graph.refresh()}catch(c){console.log("duplicate append")}}function partialGraph(a){noRefresh=!0;for(var b=0;b<a.length;++b){a[b]=a[b].split(" ");for(var c=0;c<a[b].length;++c)"+"==a[b][c]&&addRelationIndices(b,c)}graph.refresh();noRefresh=!1};
