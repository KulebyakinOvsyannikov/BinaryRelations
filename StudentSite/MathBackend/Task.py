    ...
    def generate_demo_strings(self):
        ...
        for elem in self.elements:
            for elem2 in self.elements:
                res.append("Рассмотрим элементы %s и %s" % (elem, elem2))
                if self.solve_for_elements(elem,elem2):
                    res[i] += " Видим, что они состоят в отношении R. Отмечаем соответствующий элемент матрицы"
                else:
                    res[i] += " Видим, что они не состоят в отношении R. Оставляем соответствующий элемент " \
                                 "матрицы нетронутым."
            i += 1
        checkboxes_array = [
